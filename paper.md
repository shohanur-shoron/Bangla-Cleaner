...
Text normalization has been a subject of research and development for decades, driven primarily by the needs of TTS and ASR systems. Methodologies have evolved, but core challenges related to ambiguity and coverage persist across languages.

**Rule-Based and Finite-State Approaches:** Historically, rule-based systems have been the mainstay of text normalization [1, 3]. These systems often employ cascades of hand-crafted rules, implemented using regular expressions or, more formally, Finite-State Transducers (FSTs) [4, 5]. FSTs provide a powerful and efficient mechanism for defining complex rewrite rules and have been successfully applied to morphological analysis and normalization tasks in various languages [Placeholder Reference]. Companies like Google and AT&T developed sophisticated rule-based normalization systems based on FSTs, demonstrating their scalability and effectiveness for commercial TTS systems [10, Placeholder Reference]. Their strength lies in precision and the ability to encode explicit linguistic knowledge. However, developing and maintaining large rule sets requires significant expertise and can be brittle when faced with unexpected input variations.

**Normalization for Indic Languages:** Research specific to Indic languages, including Bengali, often highlights the necessity of adapting normalization techniques to handle unique script properties, complex morphology, and distinct numerical systems [6, 7, Placeholder Reference]. Work on Hindi, Malayalam, Tamil, and other languages has explored rule-based and hybrid methods to address challenges like consonant conjuncts, agglutination, variations in date/time formats, and the lakh/crore numbering convention [Placeholder Reference]. However, comprehensive, publicly detailed systems covering the full spectrum of normalization categories specifically for Bengali remain relatively uncommon.

**Machine Learning Approaches:** The rise of deep learning has introduced sequence-to-sequence (Seq2Seq) models, including those based on Recurrent Neural Networks (RNNs) and Transformers, to the text normalization task [8, 9, Placeholder Reference]. These models learn the mapping from input NSW sequences to their normalized output sequences directly from large parallel corpora. They have shown impressive results for English, demonstrating an ability to handle diverse contexts and potentially generalize better to unseen patterns than purely rule-based systems [Placeholder Reference]. However, their performance is heavily contingent on the availability of substantial, high-quality training data. Creating such datasets for languages like Bengali is a significant bottleneck. Moreover, these models can sometimes produce linguistically implausible outputs or struggle with maintaining consistency for specific numerical or grammatical rules without sophisticated training strategies or architectural modifications [Placeholder Reference]. Hybrid approaches, combining ML with rule-based components or FSTs, have also been explored to leverage the strengths of both paradigms [Placeholder Reference].

**Positioning the Current Work:** Our work contributes a detailed description and implementation of a comprehensive *rule-based* system specifically for Bengali. While acknowledging the advancements in ML, we focus on demonstrating the power and practicality of a linguistically informed, rule-driven approach, particularly relevant given Bengali's specific challenges and the current data availability landscape. We emphasize modularity, interpretability, and coverage of Bengali-specific phenomena, including phonetic conversion, positioning the system as a valuable tool and a strong baseline for future comparative studies involving ML techniques for Bengali text normalization.
...
[9] Lusetti, M., et al. (2021). Transformer-based Text Normalization. *arXiv preprint arXiv:2110.07150*.
